...
public class PhotoDownloader {
    ...
    public Observable<Photo> searchForPhotos(String searchQuery) {
        return Observable.create(observer -> {
            try {
                log.info("searchForPhotos on thread: " + Thread.currentThread().getName());
                ...
                for (String photoUrl : photoUrls) {
                    Photo photo = null;
                    try {
                        photo = getPhoto(photoUrl);
                    } catch (IOException e) {
                        log.log(Level.WARNING, "Could not download a photo " + photoUrl);
                    }
                    if (photo != null) {
